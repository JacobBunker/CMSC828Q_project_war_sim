...
void match(GA *ga,int Ngame){
  ...
  for(int i=0;i<Ngame;++i){
    ...
    for(int j=0;j<ga->n/2;++j){
      outcome=game_of_ttt(ga->pop[schedule[2*j]],ga->pop[schedule[2*j+1]]);
      /*  outcome=0 if pop[schedule[2*j]] wins, 1 if pop[schedule[2*j+1]] wins */
      /* and 2 if draw */
      switch (outcome){
      case 0:
	ga->copy_fit[schedule[2*j]]+=1;
	ga->copy_fit[schedule[2*j+1]]-=1;
	/* ++ga->fit_array[schedule[2*j]]; */
	/* --ga->fit_array[schedule[2*j+1]]; */
	break;
      case 1:
	ga->copy_fit[schedule[2*j]]-=1;
	ga->copy_fit[schedule[2*j+1]]+=1;
